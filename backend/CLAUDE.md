...
- **`PriceUpdate`** — Immutable dataclass: `ticker`, `price`, `previous_price`, `timestamp`, `session_open` (per-ticker daily baseline, set once on first sight), plus properties `change`, `change_percent`, `direction` ("up"/"down"/"flat"), `day_change`, `day_change_percent` (measured from `session_open`), and `to_dict()` for JSON serialization.
