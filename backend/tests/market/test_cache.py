...
class TestPriceCache:
    ...
    def test_session_open_set_on_first_update(self):
        """First-seen price becomes the session-open baseline."""
        cache = PriceCache()
        update = cache.update("AAPL", 190.00)
        assert update.session_open == 190.00
        assert update.day_change == 0.0

    def test_session_open_carried_forward(self):
        """Baseline stays fixed across subsequent updates; daily change tracks it."""
        cache = PriceCache()
        cache.update("AAPL", 190.00)  # baseline = 190.00
        cache.update("AAPL", 191.00)
        update = cache.update("AAPL", 192.00)
        assert update.session_open == 190.00
        assert update.day_change == 2.00
        assert update.day_change_percent == 1.0526  # (2.0 / 190.00) * 100

    def test_explicit_session_open_on_first_update(self):
        """An explicit baseline (e.g. a real data source's day open) is used when first seen."""
        cache = PriceCache()
        update = cache.update("AAPL", 190.00, session_open=185.00)
        assert update.session_open == 185.00
        assert update.day_change == 5.00

    def test_explicit_session_open_ignored_after_first(self):
        """Once set, the baseline is not overwritten by a later explicit value."""
        cache = PriceCache()
        cache.update("AAPL", 190.00, session_open=185.00)
        update = cache.update("AAPL", 191.00, session_open=999.00)
        assert update.session_open == 185.00
